...
typedef struct _gameState{
  int playerMoney;
  player ctfer;
  char name[NAMEBUFFLEN];       // buffer overflow?
  size_t deckSize;
  player opponent;
} gameState;
...
void shuffle(deck * inputDeck){
    ...
    // 1000 times: swap two random cards from the same deck
    for(unsigned int i=0; i < 1000; i++){
        indexA = rand() % deckSize;         // overwrite decksize to 1 to stop shuffling somehow?
...
int checkInvalidCard(card * inputCard){
    /// BUG!
    // suit can be 4 but only suits of 3 are generated
...
unsigned int readInput(char * buff, unsigned int len){
    size_t count = 0;
    char c;
    while((c = getchar()) != '\n' && c != EOF){ // inject null-byte => trailing payload
        if(count < (len-1)){
            buff[count] = c;
            count++;
        }
    ...
    // BUG:
    // after reading (len-1) chars, count will be ==(len-1)
    // writing to count+1 will write to len, which is after the buffer
    // => deckSize will be 0
    buff[count+1] = '\x00';
    return count;
}
...
int main(int argc, char**argv){
    ...
    // set PRG seed to current date and time (down to seconds)
    ...
    // overwriting one decksize affects the other, if lowered
    ...
    while(1){
        ...
        if((playCard->value * 4 + playCard->suit) > (oppCard->value * 4 + 
                    /// BUG!
                    playCard->suit)){
                    /// BUG!
                    // 8,0 vs 8,3 scores tie
                    // suit is completely useless, only value counts
                    // still, the player loses on a tie
            printf("You won? Hmmm something must be wrong...\n");
            if(checkInvalidCard(playCard)){ // modified card cant be over 14,4 (slight bug)
        ...
        if(gameData.playerMoney <= 0){
            ...
            exit(0);
        }else if(gameData.playerMoney > 500){       // overwriting money directly wins as well
